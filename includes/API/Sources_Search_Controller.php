...
class Sources_Search_Controller extends Abstract_REST_Controller {
	...
	public function callback( WP_REST_Request $req ): WP_REST_Response|WP_Error {
		...
		if ( empty( $query ) ) {
			return new WP_Error( 'blig_missing_query', 'A search query is required.', array( 'status' => 400 ) );
		}
		...
		try {
			...
			return new WP_Error( 'blig_search_failed', 'Search failed.', array( 'status' => 500 ) );
		}
